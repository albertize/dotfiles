...
links=(
  ...
  "$config_home/dotfiles-runtime/gtkrc-2.0|$HOME/.gtkrc-2.0"
...
link_file() {
  ...
  if [[ $relative_source == /* ]]; then
    ...
    source="$repo_dir/$relative_source"
  fi
...
install_desktop_runtime_configs() {
  ...
  local theme_config="$config_home/dotfiles-theme/theme.conf"
  local FONT_FAMILY GTK_THEME ICON_THEME
  ...
  # shellcheck disable=SC1090
  source "$theme_config"
  : "${FONT_FAMILY:?}" "${GTK_THEME:?}" "${ICON_THEME:?}"
  mkdir -p -- "$runtime_dir"
  ...
  cat > "$runtime_dir/gtkrc-2.0" <<EOF
# Generated by the dotfiles theme and font profile switchers.
gtk-button-images=1
gtk-cursor-blink=1
