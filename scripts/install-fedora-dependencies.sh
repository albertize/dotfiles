...
# Install the Fedora packages and user-local fonts required by this repository.
set -euo pipefail
...
readonly meslo_font_archive='Meslo.tar.xz'
readonly meslo_font_sha256='6b6624632dc6873dfb7681c3f818e7c01ab601ab707690b6440933bbe57e2b11'
readonly meslo_font_url="https://github.com/ryanoasis/nerd-fonts/releases/download/v${nerd_font_version}/${meslo_font_archive}"
readonly data_home=${XDG_DATA_HOME:-"$HOME/.local/share"}
readonly font_dir="$data_home/fonts/JetBrainsMonoNerdFont"
readonly font_version_file="$font_dir/.version"
readonly meslo_font_dir="$data_home/fonts/MesloLGSNerdFont"
readonly meslo_font_version_file="$meslo_font_dir/.version"
...
meslo_font_installed=false
if [[ -r $meslo_font_version_file ]] &&
   [[ $(<"$meslo_font_version_file") == "$nerd_font_version" ]]; then
  meslo_font_installed=true
fi
...
if $meslo_font_installed; then
  printf 'MesloLGS Nerd Font %s is installed in %s.\n' \
    "$nerd_font_version" "$meslo_font_dir"
else
  printf 'MesloLGS Nerd Font %s will be installed in %s.\n' \
    "$nerd_font_version" "$meslo_font_dir"
fi

if $check_only; then
  if (( ${#missing_packages[@]} > 0 )) || ! $font_installed || ! $meslo_font_installed; then
    exit 1
  fi
  exit 0
fi
...
if ! $font_installed || ! $meslo_font_installed; then
  tmp_dir=$(mktemp -d)
...
if ! $meslo_font_installed; then
  curl --fail --location --silent --show-error \
    "$meslo_font_url" --output "$tmp_dir/$meslo_font_archive"
  printf '%s  %s\n' "$meslo_font_sha256" "$tmp_dir/$meslo_font_archive" |
    sha256sum --check --status

  install -d -m 0755 -- "$meslo_font_dir"
  rm -f -- "$meslo_font_dir"/*.ttf
  tar -xJf "$tmp_dir/$meslo_font_archive" -C "$meslo_font_dir" \
    MesloLGSNerdFont-Regular.ttf \
    MesloLGSNerdFont-Bold.ttf \
    MesloLGSNerdFont-Italic.ttf \
    MesloLGSNerdFont-BoldItalic.ttf
  printf '%s\n' "$nerd_font_version" > "$meslo_font_version_file"
  fc-cache -f "$meslo_font_dir" >/dev/null
fi
